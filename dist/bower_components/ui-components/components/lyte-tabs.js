...
_observedAttributes :["ltPropYield","ltPropHover","ltPropActiveClass","ltPropPosition","ltPropOpenTabOn","ltPropNewContent","ltPropCloseIcon","prevTarget"],
	...
	initialFunc : function(){
		...
		if(this.$node.childElementCount > 1 || this.$node.children[0].tagName === "LYTE-TAB"){
			...
			var labels = this.$node.querySelectorAll('lyte-tab-title');
			var contents = this.$node.querySelectorAll('lyte-tab-content');
			var pos;
			...
			if(this.getData('ltPropCloseIcon')){
				this.createCloseIcon(this.$node.querySelectorAll('lyte-tab-title'));
			}
	...
	showTab : function(event){
		...
		if($L(event.target).hasClass('lyteTabCloseIcon')){
			...
			target = (this.$node.querySelectorAll('lyte-tab-title').length > 0) ? this.$node.querySelectorAll('lyte-tab-title')[0] : null;
			if(!target){
				return
			}
	...
	deleteTabContent : function(tabId){
		if(tabId){
			...
			var headers = this.$node.querySelectorAll('lyte-tab-title');
	...
	customizeTitleTab : function(prop){
		if(prop === "top" || prop === "bottom"){
			...
			var titles = this.$node.querySelectorAll('lyte-tab-title');
