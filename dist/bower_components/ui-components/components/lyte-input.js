...
_observedAttributes :["ltPropDisabled","ltPropAutofocus","ltPropAutocomplete","ltPropMaxlength","ltPropName","ltPropPlaceholder","ltPropReadonly","ltPropValue","ltPropWidth","ltPropInterval","ltPropTimeFormat","ltPropHourInterval","ltPropRows","ltPropDefaultTime","ltPropCols","ltPropMinuteInterval","ltPropStartdate","ltPropEnddate","ltPropCurrentdate","ltPropMonthheader","ltPropType","ltPropAppearance","ltPropDirection","ltPropLabel","ltPropId","ltPropClass","ltPropAccept","ltPropMultiple","ltPropText","ltPropFilelist","ltPropMax","ltPropMin","ltPropStep","ltPropStyle","ltPropDropdown","ltPropStartTime","ltPropEndTime","iniHour","iniMinute","iniMeridian","finalHour","finalMinute","finalMeridian","ltPropText","fileNo","ltPropTextAreaResize","ltPropHeight","ltPropFlag","ltPropFormat","ltPropYear","ltPropMonthHeaderFormat","daysOfWeek","viewDate","drop1data","drop2data","drop3data","eventListeners","ltPropBoundary"],
	...
	didConnect:function(){
		   if(this.getData('ltPropType') == 'date')
		    	{
		    		this.$node.calendarDiv.addEventListener('click',function(event){
	...
	didDestroy : function(){
		...
		if(this.getData('ltPropType') == 'date'){
			document.removeEventListener('click', this.getData('eventListeners').calendar);
	...
	calenderClickHide : function(event){
			if(event.target != $L('input', this.$node).e[0])
		    	{
	...
	scrollFunc : function(event){
		var wormholeCalendar = this.$node.calendarDiv;
		...
		if(!wormholeCalendar.classList.contains('lyteCalendarHidden'))
			{
				var nodeN = $L('input',this.$node).e[0]
 	     		wormholeCalendar.style.left = (nodeN.getBoundingClientRect().left + xscroll) + 'px';
 	     		wormholeCalendar.style.top = (nodeN.getBoundingClientRect().top +yscroll + nodeN.offsetHeight) + 'px';
				...
				if((clientRect.left < (boundary.hasOwnProperty('left') ? boundary.left : 0)) || (clientRect.right > (boundary.hasOwnProperty('right') ? boundary.right : window.innerWidth)) || (clientRect.top < (boundary.hasOwnProperty('top') ? boundary.top : 0)) || (clientRect.bottom > (boundary.hasOwnProperty('bottom') ? boundary.bottom : window.innerHeight)))
					{	
						wormholeCalendar.classList.add('lyteCalendarHidden');
					}
 	     	}
	},
  		...
  		if(this.getData('ltPropType') != 'textarea')
	  		 {
			     ...
			     input.style.height = this.getData('ltPropHeight')
			     input.style.padding = window.getComputedStyle(lyteFieldDiv, null).getPropertyValue('padding')
			     lyteFieldDiv.style.padding = '0px'
			     if(this.getData('ltPropType') == 'search'){
		   			var span = $L('span.searchIcon', this.$node).e[0];
		   			span.style.top = (input.clientHeight/2 - span.clientHeight/2) + 'px'
	   			}
	   		}
	   	else
	   		{
  	...
  	appearanceFun : function () {
  		...
  		if(this.getData('ltPropAppearance')=='box')
   		...
   		else
   			{
	   			...
	   			var lyteInputBox = $L('.lyteInputBox', this.$node).e[0]	
	   			if(lyteInputBox)
	   				{
	...
	actions: {
			 wormholeDidConnect : function(){
	 	     ...
	 	     "keyup":function(event){
	 	     	if(this.getData('ltPropType')!="time"&&this.getData('ltPropType')!="textarea")
	 	     		{
	 	     			this.setData('ltPropValue',$L('input',this.$node).e[0].value);
	 	     		}
	 	     	else if(this.getData('ltPropType')!="textarea")		
	 	     		{
	 	     ...
	 	     "onChange":function(event){
                if(this.getMethods("onChange"))
	 	         	{
	 	         		this.executeMethod('onChange', event)
	 	         	}
	 	     },
				 	     		...
				 	     		if(thisCalendar.classList.contains('lyteCalendarHidden'))
				 	     			{
				 	     				...
				 	     				thisCalendar.style.left = (input.getBoundingClientRect().left + xscroll) +'px';
				 	     				thisCalendar.style.top = (input.getBoundingClientRect().top + input.offsetHeight + yscroll)+'px';
				 	     			}
				 	     	}
			 	     if(this.getMethods("onFocus"))
			 	     	  {
