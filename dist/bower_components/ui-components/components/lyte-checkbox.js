...
_template:"<template tag-name=\"lyte-checkbox\">\t<template is=\"switch\" value=\"{{ltPropType}}\">\t\t<template case=\"default\">\t\t\t<label class=\"lyteCheckbox lyteDefault\" onmouseup=\"{{action('mup',event)}}\">\t\t\t\t<input type=\"checkbox\" id=\"{{ltPropId}}\" name=\"{{ltPropName}}\" value=\"{{ltPropValue}}\" checked=\"{{unbound(ltPropChecked)}}\" disabled=\"{{ltPropDisabled}}\" class=\"\" readonly=\"{{ltPropReadonly}}\" onclick=\"{{action('checkBoxClicked',event)}}\">\t\t\t\t<span class=\"{{ltPropClass}}\">\t\t\t\t\t<span class=\"{{ltPropLabelClass}}\">\t\t\t\t\t\t{{ltPropLabel}}\t\t\t\t\t</span>\t\t\t\t</span>\t\t\t</label>\t\t\t</template>\t\t<template case=\"primary\">\t\t\t<label class=\"lyteCheckbox lytePrimary\" onmouseup=\"{{action('mup',event)}}\">\t\t\t\t<input type=\"checkbox\" id=\"{{ltPropId}}\" name=\"{{ltPropName}}\" value=\"{{ltPropValue}}\" checked=\"{{unbound(ltPropChecked)}}\" disabled=\"{{ltPropDisabled}}\" class=\"\" readonly=\"{{ltPropReadonly}}\" onclick=\"{{action('checkBoxClicked',event)}}\">\t\t\t\t<span class=\"{{ltPropClass}}\">\t\t\t\t\t<span class=\"{{ltPropLabelClass}}\">\t\t\t\t\t\t{{ltPropLabel}}\t\t\t\t\t</span>\t\t\t\t</span>\t\t\t</label>\t\t\t</template>\t\t<template case=\"switch\">\t\t\t<label class=\"\" onmouseup=\"{{action('mup',event)}}\">\t\t\t\t<input type=\"checkbox\" id=\"{{ltPropId}}\" name=\"{{ltPropName}}\" value=\"{{ltPropValue}}\" checked=\"{{unbound(ltPropChecked)}}\" disabled=\"{{ltPropDisabled}}\" class=\"lyteHide on-off-sw\" readonly=\"{{ltPropReadonly}}\" onclick=\"{{action('checkBoxClicked',event)}}\">\t\t\t\t<span class=\"{{ltPropClass}}\">\t\t\t\t\t<span class=\"on-btn\"></span>\t\t\t\t</span>\t\t\t\t<span class=\"{{ltPropLabelClass}}\">\t\t\t\t\t{{ltPropLabel}}\t\t\t\t</span>\t\t\t</label>\t\t\t</template>\t\t<template case=\"slider\">\t\t\t<label class=\"lyteCheckSliderLabel\" onmouseup=\"{{action('mup',event)}}\">\t\t\t\t<input type=\"checkbox\" id=\"{{ltPropId}}\" name=\"{{ltPropName}}\" value=\"{{ltPropValue}}\" checked=\"{{unbound(ltPropChecked)}}\" disabled=\"{{ltPropDisabled}}\" class=\"lyteHide\" readonly=\"{{ltPropReadonly}}\" onclick=\"{{action('checkBoxClicked',event)}}\">\t\t\t\t<span class=\"{{ltPropClass}}\">\t\t\t\t\t<span class=\"{{ltPropLabelClass}}\">\t\t\t\t\t\t{{ltPropLabel}}\t\t\t\t\t</span>\t\t\t\t</span>\t\t\t\t\t\t\t</label>\t\t\t</template>\t</template></template>",
...
_observedAttributes :["ltPropType","ltPropId","ltPropDisabled","ltPropChecked","ltPropLabel","ltPropName","ltPropValue","ltPropReadonly","ltPropFireOnInit","ltPropClass","ltPropColor","ltPropLabelClass"],
data:function(){
    return {
...
didConnect:function(){
	this.$node.constructor._observers[0].value.call(this,undefined,true)
},
fireCallbacks:function(arg1,onrender){
	if(this.getData('ltPropChecked').toString().toLowerCase() == 'true'){
		if(this.getData('ltPropType') == 'switch'){
			this.$node.querySelector('span').style.background = this.getData('ltPropColor')
		}
...
actions:{
	...
	checkBoxClicked:function(event){
		...
		if(checked==false){
			if(this.getData('ltPropType') == 'switch'){
				this.$node.querySelector('span').style.background = ''
			}
