...
_observedAttributes :["ltPropAvailablelist","ltPropSelectedlist","ltPropMultiselect","ltPropEnableSearch","ltPropEnableIcon","ltPropDraggable","ltPropErrorMessage","ltPropAvailablelistEmptyMessage","ltPropSelectedlistEmptyMessage","clickCount","component","placeholder","div","offset","isDown","isMoved","top","bottom","prevScrollTop","multiSelectedItems","keyPressed","keyValue","prevMode","checkSelection","querySelector","left","right","availableList","selectedList","checkOnDidconnect"],
	didConnect : function() {
		var dragdrops = document.querySelectorAll('lyte-dragdrop');
		for(var v = 0 ; v < dragdrops.length; v++){
			if(dragdrops[v] == this.$node){
				// console.log(v);
				this.$node.querySelector('#sortable').classList.add('searchable'+v);
	...
	data : function(){
        return {
			...
			"querySelector" : Lyte.attr("object",{"default":{'scope':'#sortable','target':'.filterlabel','search':'.item'}}),
	...
	makeDroppable : function(current){
		var dropables1 = this.$node.querySelectorAll('#sortable>li');
	...
	generateArray : function(){
		...
		var array1 = Array.from(this.$node.querySelectorAll("#sortable>li"));
	...
	makeDroppableMultiple : function(array){
		var dropables1 = Array.from(this.$node.querySelectorAll('#sortable>li'));
	...
	checkElement : function(ele){
		if((ele == this.$node.querySelector('#sortable')) || (ele == this.$node.querySelector('#dropSec'))){
			return true;
		}
		return false;
	},
	...
	checkIfDroppable : function(elem,prop){
		if(prop === "multiple"){
			var multiSelectedItems = this.getData('multiSelectedItems');
			if((multiSelectedItems[0].parentElement.id === elem.id) && elem.id === "sortable" && (elem.querySelectorAll('li').length === multiSelectedItems.length) && multiSelectedItems[0].getBoundingClientRect().left < (elem.getBoundingClientRect().left + multiSelectedItems[0].getBoundingClientRect().width / 2)){
				return true;
			}
		...
		if(prop === "single"){
			...
			if((div.parentElement.id === elem.id) && elem.id === "sortable" && (elem.querySelectorAll('li').length === 1) && div.getBoundingClientRect().left < (elem.getBoundingClientRect().left + div.getBoundingClientRect().width / 2)){
				return true;
			}
	...
	appendElement : function(event,prop){
		...
		if(prop == 'span'){
			...
			if(elem.classList.contains('lyteDragDropRemove') && this.onClickFunction(event,this.getElement(elem.parentElement),"selectedList")){
				// this.$node.querySelector('#sortable').appendChild(elem.parentElement);
				element = this.getElement(elem.parentElement);
			...
			while(elem){
				if(elem.parentElement && elem.parentElement.id == 'sortable' && this.onClickFunction(event,this.getElement(elem),"availableList")){
	...
	selectFunction : function(event,target){
		var fromList = target.parentElement.id == "sortable" ? "availableList" : "selectedList"; 
		var returnVal;
	...
	onDropFunction : function(event){
		if(this.getMethods("onDrop")){
			this.executeMethod("onDrop",event,this.getData('availableList'),this.getData('selectedList'));	
		}
	},
	...
	onClickFunction : function(event,element,fromList){
	 ...
	 actions : {
				...
				if(this.getData('ltPropMultiselect')){
		 			if(multiSelectedItems.length > 0 && (multiSelectedItems.indexOf(target) != -1)){
						this.setData('offset',[
					        ...
					        event.pageY - target.getBoundingClientRect().top
						]);
		...
		mouseOver : function(event){
			...
			if(target.classList.contains('lyteDraggableElem')){
				if(target.parentElement.id == "sortable"){
		...
		if(component.getData('isMoved')){
			...
			if(returnVal){
				if(component.getData('ltPropMultiselect') && component.getData('multiSelectedItems').length > 1 ){
					...
					// console.log(elem);
					var parentListOfObjects = multiSelectedItems[0].parentElement.id == "sortable" ? "ltPropAvailablelist" : "ltPropSelectedlist";
					var parentListOfPlaceholder = placeholder.parentElement.id == "sortable" ? "ltPropAvailablelist" : "ltPropSelectedlist";
					var objects = [];
					for(var v = 0; v <multiSelectedItems.length ; v++){
					...
					var placeholder = component.getData('placeholder');
					var prevEle = placeholder.previousElementSibling;
					var parentListOfDiv = div.parentElement.id == 'sortable' ? "ltPropAvailablelist" : "ltPropSelectedlist";
					var parentListOfPlaceholder = placeholder.parentElement.id == 'sortable' ? "ltPropAvailablelist" : "ltPropSelectedlist";
					while(prevEle && prevEle.tagName == "TEMPLATE"){
						prevEle = prevEle.previousElementSibling;
					}
					...
					if(component.$node.querySelector('#sortable').contains(component.$node.querySelector('#dummy'))){
			    		component.$node.querySelector('#sortable').removeChild(component.$node.querySelector('#dummy'));
			    	}
		...
		// if(component.getData('ltPropDraggable')){
			if(component.getData('ltPropMultiselect') && component.getData('multiSelectedItems').length > 1){
		        ...
		        if(totalHeight > placeholder.getBoundingClientRect().height){
		        	if(document.getElementById('sortable').contains(document.getElementById('dummy'))){
			    		document.getElementById('sortable').removeChild(document.getElementById('dummy'));
			    	}
		        ...
		        if(!component.getData('left') && dropablePlaceLeft && (div.getBoundingClientRect().left < (dropablePlaceLeft.getBoundingClientRect().right - div.getBoundingClientRect().width / 2))){
					...
					if(top < prevTop){
		        		...
		        		if((elemBelow.id === "sortable" || elemBelow.id === "dropSec") && component.checkIfDroppable(elemBelow,"multiple")){
		  ...
		  //       console.log("top ====>" + component.getData('placeholder').getBoundingClientRect().top);
			...
			/*---Single drag and drop----*/
		        ...
		        if(droppableLeft && (div.getBoundingClientRect().left < (droppableLeft.getBoundingClientRect().right - maxAllowableWidth))){
		        	...
		        	if((elemBelow.id === "sortable" || elemBelow.id === "dropSec") && component.checkIfDroppable(elemBelow,"single")){
