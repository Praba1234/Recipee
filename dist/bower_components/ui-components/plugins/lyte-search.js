(function(){
    if($L){
        $L.prototype.search = function(data){
            ...
            var scope = typeof data.scope == 'string' ? ((/^#/g.test(data.scope.trim()) && !/\s/g.test(data.scope.trim()))? $L(data.scope).e : $L(data.scope).e[0]) : data.scope, searchComp = [], searchList = [], targetList = [], timeout, method = data.method ? data.method : 'contains';    
      ...
      findingList = function(){
            searchList = [], targetList = [], searchComp = [];
            if(data.search)   
                {
