...
_observedAttributes :["ltPropScope","ltPropHandler","ltPropContent","ltPropMarginLeft","ltPropMarginTop","ltPropUnit","ltPropGridLength","ltPropResizeDirection","ltPropFloat","ltPropDirection","ltPropUndo","ltPropResize","ltPropBestfit","lyteGridStack","ltPropFlag","elements","oriNode","xElements","yElements","lyteQuerySelector","lyteGridstackKeyFlag"], 
    didDestroy : function(){
    ...
    // collecting data and calculating max height and width of the scope
        ...
        $L(this.getData('ltPropContent'),scope).addClass('lyteGridStackContent')
        scope.dataSet = {};
        ...
        for(var i = 0; i < elements.length; i++ )
            {
                ...
                if(this.getData('ltPropResize') == true)
                    {
                        if(data[i].nodeName.getAttribute('lt-prop-resize') != "disabled")
                            {
        ...
        lyteQuerySelector.MaxLength = Math.max(...a); 
        lyteQuerySelector.MaxHeight = Math.max(...b); 
        lyteQuerySelector.MaxBottom = Math.max(...c);
        ...
        if(!styleFlag)    
                {
                    if($L('style#lyteGridStack',this.$node).e[0] == null)     
                        {
                    ...
                    style = $L('style#lyteGridStack',this.$node).e[0]
                    var elem = this.getData('ltPropScope')+this.getData('ltPropHandler') + ".selected.lyteSizeChange{transition: width 0s, height 0s;}"
                    style.append(elem)
                    elem=this.getData('ltPropScope') + "{overflow-anchor : none; overflow : hidden !important ; outline : none;}"
                    style.append(elem)
                    elem = this.getData('ltPropScope') + " " + this.getData('ltPropHandler') + ".items:not(.selected){position: absolute; transition-duration: 0.4s; overflow: auto; -ms-overflow-style: none; cursor : move;}"
                    style.append(elem)
                    elem = this.getData('ltPropScope') + " " + this.getData('ltPropHandler') + ".selected{opacity: 0.4;z-index: 90;position: absolute; cursor : move;}"
                    style.append(elem)
    ...
    maxHeight : function(data){
            ...
            return [Math.max(...a), Math.max(...b), Math.max(...c)] 
    }, 
    ...
    positionFind : function(elements){
                ...
                Max = Math.max(Math.max(...x), Math.max(...y), Math.max(...a), Math.max(...b));
                for(var j = 0;j <= Max;j++ )
                    {
                        x = [], y = [], a = [], b = [];
    ...
    nodeName : function(event){
            ...
            if(nodeName)
            {
                ...
                while(nodeName != element && nodeName.tagName != "BODY")
                    {
                        ...
                        else if(nodeName.classList.contains('lyteGridStackContent'))
                            {
                                val = 'content'
                                flag = true
                                break;
    ...
    var lyteQuerySelector = this.getData('lyteQuerySelector')
    for(var z = lyteQuerySelector.SelectedNodes.length-1;z >= 0;z--)
        {   
                    ...
                    if((node.value == "left" || node.value == "BottomLeft") && (node.xOff + Math.abs(xPos-node.getBoundingClientRect().left-node.clientWidth)) >= parseInt(unit))
                        {
                            ...
                            data[i].x1 = parseInt(Math.ceil((node.offsetLeft + node.clientWidth-parseInt(unit))/(parseInt(unit) + parseInt(MarginLeft))).toFixed(0))
            ...
            for(i = 0;i < elementsToCheck.length;i++ )
             ...
             if(topElements.length)
                {   
                    ...
                    ht = totalHeight;
                    var elem = [], elem1 = [];
                    totalHeight = Math.max(...maxElem) - data[topElements[0]].y1 + ht   
                        for(var j = 0;j < topElements.length;j++ )
                            {
                ...
                if(ret)
                    {
                        ...
                        if(!evt.shiftKey)   
                            ...
                            {
                                ret = this.nodeName.call(this, event);
                                if(ret)
                                    {
                                        nodeName = ret[1]
                                        nodeName.initialLeft = [], nodeName.initialTop = []
...
// after rendering properties
    ...
    data : function(){
        return {
            ...
            ltPropContent : Lyte.attr('string',{'default' : ''}),
