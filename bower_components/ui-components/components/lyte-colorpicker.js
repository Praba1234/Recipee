	...
	initializeAdvColorPicker : function(event,ele){
		...
		var checkImageLoaded = setInterval(function(){
			if(imageObj.complete){
				...
				colorDiv.addEventListener('mousedown',function(event){
					var ele = ColorPicker_Util.getColorPicker(event.target);
	...
	actions : {
		...
		goToAdvancedCP : function(event){
			...
			if(popOver.querySelector('lyte-popover-header')){
				popOver.style.height = "436px";
			}else{
				popOver.style.height = "390px";
			}
			popOver.style.maxWidth = "315px";
			popOver.querySelector('lyte-popover-content').style.height = "390px";
		...
		fromAdvCPtoBasic : function(event){
			...
			this.initializeBasicColorPicker(event,document.querySelector('.'+this.getData('ltPropWrapperClass')));
			popOver = null;
		},
		...
		onOverColorPreviewDiv : function(event, type, prop){
			...
			if(type === "fromBasic"){
				if(prop === "over"){
					document.querySelector('#basciPrevieDivImg').style.display = "block";
				}
				if(prop === "out"){
					document.querySelector('#basciPrevieDivImg').style.display = "none";
				}
			}
		}
	}
