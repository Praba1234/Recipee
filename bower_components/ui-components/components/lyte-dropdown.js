//We are using displayvalue to show ltPropSelected in case of singular select because selected might be affected by both ltPropOptions and ltPropSelected itself
//selected isn't changing back to undefined => REALLY IMPORTANT FIX
/*
/* 
Fixes for these issues are not known:
1.There is a dropdown which has freezelayer set to true.Opening it causes body's overflow to be hidden.
body's old overflow value is stored.But if the overflow value of the body is changed to something else by the developer
when the dropdown is open,closing it will reflect the old overflow value.Body overflow is set to hidden to prevent body scroll.
2.We need to set CSS for the dropdown container when the length of the items changes so that it goes above or below the
dropdown.This is not possible in yield.
3.Setting custom position for dropdown in multiselect
*/
/* Review:
SetProperWidth just straight up sucks
performance improvement or atleast code revamp needed for dropdown first item selected
modularize code more 
*/
/* FIXME:
-3.Awful fix done to multisearch enter press to go to the next element.
4.dropdown causing problems when body.overflow is applied.This is causing problems in input
9.callout true with hover is not working properly(all cases)
2.fix lyteArrow class
3.setcss multiselectcss(code help this is for multiselect) will cause reflow
4.maybe width keeps on adding when opening a dropdown
*/
/* TESTME:
1.All positions for basic dropdown with and without callout.(complete)
2.All positions for multiselect dropdown with and without callout.(There is probably only down)(complete)
3.All positions for multisearch dropdown with and without callout.(complete)
4.All positions for hover basic dropdown with and without callout.(complete)
5.All positions for open dropdown with and without callout.(hover dropdowns shouldn't open in the start.Just doesn't make sense)(complete)
6.ltPropSelected and ltPropOptions given at the start.(all dropdowns)(complete for both yielded and non-yielded dropdowns)
7.Both not given at the start.(all dropdowns)(complete for both yielded and non-yielded dropdowns)
8.only ltpropselected at the start.(all dropdowns)(complete for both yielded and non-yielded dropdowns)
9.only ltpropoptions given at the start.(all dropdowns)(complete for both yielded and non-yielded dropdowns)
10.both given later.(all dropdowns)
11.do above for all datatypes.
12.do above for yield dropdowns.
13.Make sure disabled dropdowns don't open for
	13.1.1 $node.toggle(complete)
	13.1.2 tabbing (should not focus)
	13.1.3 clicking (no focus)
14.$node.toggle should focus the dropdown
15.Test normal search dropdown
	15.1.1 backspace/bad characters/up/down/enter
16.Test multisearch dropdown
	16.1.2 backspace/bad characters/up/down/enter
17.Make sure footer works for everything.
18.open dropdowns should scroll the user to its position.
19.Test with and without web inspector.
20.Test non-freeze dropdown
21.Data comes afterwards both ltpropselected and ltpropoptions.
22.dropdown can't go above or below in setcs might fix it at intial position
*/
/*
Design thoughts:
1.Do you need hover for multiselect.Say you select the last element in a hover multiselect it disappers and the dropdown closes.
2.OnBeforeClose works on return false but it changes the ltpropselected when it is clicked on lyte-drop-item.So the ltpropselected changes and it doesn't close so it feels weird.
*/
LyteComponent.registerComponent('lyte-dropdown',{
		...
		if(this.getData('ltPropPosition') == 'down'){
			...
			if(downflag){
				if(this.getData('ltPropCallout')){
					...
					elem1.style.top = this.setTopBelowForDown(parNode,bcr,yscroll) + 9 + 'px'
				...
				if(this.getData('ltPropCallout')){
					...
					elem1.style.top = this.setTopAboveForDown(parNode,elem1,bcr,elembcr,yscroll) - 9 + 'px'
				}
				else {
			...
			if(rightflag){
				if(this.getData('ltPropCallout')){
					var num = bcr.width/2  // We removed arrow.offsetWidth because it was giving width as 0 px
				...
				if(this.getData('ltPropCallout')){
					var denom = ewidth/100
					var num = ewidth - (bcr.width/2) // We removed arrow.offsetWidth because it was giving width as 0 px
					var per = num/denom 
					arrow.style.left = per +"%" 
				}
			...
			if(rightflag){
				if(this.getData('ltPropCallout')){
					...
					elem1.style.left = this.setLeftForRight(parNode,bcr,xscroll) + 9 + "px"
				}
				else{
				...
				if(this.getData('ltPropCallout')){
					...
					elem1.style.left = this.setRightForRight(parNode,elem1,bcr,elembcr,xscroll) - 9 + "px"
				}
				else{
			...
			if(downflag){
				if(this.getData('ltPropCallout')){
					var num = ((bcr.height/2) - (4)) * 100 // Had arrow.getBoundingClientRect()/2 removed cos its value is 0
				...
				if(this.getData('ltPropCallout')){
					var num = (elembcr.height - bcr.height/2 - (4)) * 100 // Had arrow.getBoundingClientRect()/2 
					var denom = elembcr.height
					var per = num/denom
					arrow.style.top = per +"%"
				} 
			...
			if(downflag){
				if(this.getData('ltPropCallout')){
					var num = ((bcr.height/2) - (4)) * 100 // Had arrow.getBoundingClientRect()/2 
					var denom = elembcr.height
					var per = num/denom
					arrow.style.top = per + "%"
				}
	...
	actions:{
		...
		closeFun:function(event){
			...
			var data = realNode.getAttribute('data-value')
				...
				this.setData('ltPropSelected',"[" + newarr.toString() + "]")
...
window.addEventListener('scroll',function(event){	
	...
	var bottom = boundary.bottom?boundary.bottom:window.innerHeight
...
var LyteDropItem = function (_HTMLElement) {
  _inherits(LyteDropItem, _HTMLElement);

  function LyteDropItem() {
    _classCallCheck(this, LyteDropItem);

    return _possibleConstructorReturn(this, (LyteDropItem.__proto__ || Object.getPrototypeOf(LyteDropItem)).call(this));
  }

  _createClass(LyteDropItem, [{
...
customElements.define('lyte-drop-item', LyteDropItem);
...
var LyteDropGroup = function (_HTMLElement) {
    _inherits(LyteDropGroup, _HTMLElement);

    _createClass(LyteDropGroup, null, [{
        key: 'observedAttributes',
        get: function get() {
            return ['label'];
        }
    ...
    function LyteDropGroup() {
        _classCallCheck(this, LyteDropGroup);

        return _possibleConstructorReturn(this, (LyteDropGroup.__proto__ || Object.getPrototypeOf(LyteDropGroup)).call(this));
    }

    _createClass(LyteDropGroup, [{
        key: 'attributeChangedCallback',
        value: function attributeChangedCallback(attr, oldValue, newValue) {
            if (attr == 'label') {
    ...
    return LyteDropGroup;
}(HTMLElement);

// Define the new element


customElements.define('lyte-drop-group', LyteDropGroup);